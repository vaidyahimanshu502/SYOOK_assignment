...
const generateRandomMessage = () => {
  ...
  const encryptedMessage = cipher.update(JSON.stringify(payload), 'utf8', 'hex') + cipher.final('hex');
...
emitterSocket.on('connection', (socket) => {
  ...
  setInterval(() => {
    ...
    const messages = Array.from({ length: messagesCount }, generateRandomMessage);
...
async function validateAndSave(data) {
  const decipher = crypto.createDecipher('aes-256-ctr', encryptionKey);
  const decryptedMessage = decipher.update(data, 'hex', 'utf8') + decipher.final('utf8');
  ...
  try {
    ...
    await db.collection('timeSeriesData').updateOne(
      { _id: minuteStart },
